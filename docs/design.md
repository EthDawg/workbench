...
`WorkbenchQuickPanel` owns the seven fixed menu rows and receipt/editor well. `KeyboardCoachModel` remains the only shortcut editor and conflict checker. `PromptInsertionRunner` checks exact destination snapshots around every selected-text write and before the guarded `TextDelivery` paste fallback; its native adapter owns cancellation and temporary Escape monitors. `DemoPresentation.makeControlsMenu()` and `PersonaLibrary.makeControlsMenu()` return native snapshots to the shared toolbar while their existing controllers retain lifecycle ownership.
...
`CapturePanelController` also owns the persistent floating toolbar. `FloatingToolbarSurface` selects contextual tools, compact dictation/reading, narration or a temporary screenshot hide without changing the saved idle preference. Dictation captures the current Mac target at the toolbar click. Snap & Talk reuses this panel instead of creating a second recording window. A square template status icon is explicitly visible on launch; Window commands recover toolbar access when the menu bar is crowded. Debug QA bundles can provide a bounded temporary `WorkbenchFixtureRoot` to exercise these actual surfaces without using Preview/production storage; release builds ignore it.
...
Snap & Talk has its own editable shortcut. A first press captures only the display containing the pointer through ScreenCaptureKit, with the pointer included, before starting AVFoundation microphone recording. A second press commits the WAV into the user-chosen session folder and queues it through the shared recognition engine. Screenshots and audio commit before transcription, so another section can begin while recognition proceeds sequentially. Interrupted recordings remain visible as unfinished sections; queued work resumes from saved audio after relaunch. The manifest owns ordering, while UUID directories keep reordering non-destructive. `ReadbackOrdering.swift` presents a compact native table with same-table, single-use drag tokens and insertion feedback. Its draft order is separate from storage until Save; `saveSectionOrder` verifies the original active IDs/order and merges the chosen permutation into the latest section records. Cancel and hover never write. Replacement archives prior media inside the section, and deletion moves the whole section into the session's `trash` directory until explicitly emptied. Its handoff menu writes only a task prompt to the clipboard, reveals the local folder and opens a chosen installed AI app; the user still grants folder access and submits the prompt, so Workbench never silently uploads session media.
...
The Mac editor hosts `MovingSceneView` through `SceneCanvas`. `SceneMotionState` resolves temporary edit/pause state and native activity, visibility, accessibility and energy conditions into playback plus a visible reason. `SceneList` delegates selection, field editing and drag feedback to AppKit; model methods commit complete revision-checked changes. `LogoBrowser` uses an ephemeral `WKWebView` and an isolated script world for explicit image selection. `LogoWebImport` owns bounded, cancellable download validation; the existing logo store and scene model own the confirmed save. No browser credentials, search history or additional asset library are persisted.
...
`PersonaLibrary` remains the saved asset/group owner. `PersonaSessionController` in `PersonaSession.swift` owns one bounded live snapshot and a window controller per placed instance. `PersonaLibrary.makeControlsMenu()` supplies frozen public labels and ID-based actions to the shared toolbar and menu panel. Embedded Workbench suppresses the separate `PersonaHUDController`; standalone fixtures retain it for compatibility. `PersonaPresentationPreparation` owns an explicit draft and saves through the existing archive; v3 preserves an exact pre-upgrade manifest. The shell exposes the same actions through the existing StageKit shortcut catalogue and menu panel. No browser DOM, tab or URL tracking is involved. The [persona contract](personas.md) owns lifecycle, compatibility and verification.
