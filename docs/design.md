...
`DemoSnapshotStore` retains one latest video sample behind a lock. Explicit Copy snapshots its generation, converts the sample's clean aperture/pixel aspect off main and composites through `SceneRenderer` on main. The final clipboard commit rechecks generation, device/session availability and monotonic frame age. UI End/selection/reconnect invalidate immediately, before queued teardown. Scene PNG export remains static without a device image; live Copy supplies one image clipped and aspect-fitted beneath logo/persona. No controls or independent windows are captured.
