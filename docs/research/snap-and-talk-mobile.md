...
The first implementation is tracked in [issue #60: add existing images to a session](https://github.com/EthDawg/workbench/issues/60). This document holds the research and conditional follow-ons; the issue owns implementation status.
...
Run a small comparison using the same synthetic examples: the current send-to-self method, Add images with a chosen transfer, and the optional Shortcut. Observe the complete job rather than counting only capture taps. Record time to a usable section, lost or re-entered explanations, retry/confusion points and whether the result gets reused for a second artifact.
...
The next product decision is therefore small: make existing images first-class Snap & Talk inputs, then measure whether capturing their meaning on the phone is the next real bottleneck.
