...
A floating menu matters when it prevents a lost place: start the next action, stop the current one, return to software input, or move the result onward. [PR #62](https://github.com/EthDawg/workbench/pull/62) merged on 20 September; [#56](https://github.com/EthDawg/workbench/issues/56) retains acceptance work. Test closed/idle/recording states, keyboard recovery, placement and audience visibility before another menu redesign.
...
| --- | --- |
| [#56](https://github.com/EthDawg/workbench/issues/56), [#57](https://github.com/EthDawg/workbench/issues/57), [#58](https://github.com/EthDawg/workbench/issues/58), [PR #62](https://github.com/EthDawg/workbench/pull/62) | Follow-up status check: #62 merged at `c692a4b` on 20 September. Finish the specific acceptance gaps in the live issues, not the completed implementation. The earlier source audit remains tied to its stated revision. |
...
## How established projects retain decisions

Primary guidance checked 20 September 2026. These are documented practices from established projects, not proof that a process caused their success. Borrow the useful habit at Workbench's scale.

| Project/source | Practice worth keeping | Adaptation for Workbench |
| --- | --- | --- |
| [Rust RFC lifecycle](https://github.com/rust-lang/rfcs/blob/master/README.md#the-rfc-life-cycle) and [language-change policy](https://rust-lang.github.io/rfcs/lang_changes.html) | Distinguish postponement from rejection and explain both. An accepted RFC does not imply implementation priority or an assigned developer. Significant later changes reference the earlier decision. | Say whether we need evidence, are deferring, or are declining scope. Keep the reason and reconsideration condition; record actual ownership separately. No subteams or formal comment period are needed. |
| [Godot proposal guidance](https://github.com/godotengine/godot-proposals/blob/master/README.md#what-to-do-if-your-proposal-is-closed) | Search existing proposals, describe the actual problem/workaround, and link related ideas. Duplicate and rejected proposals have different meanings; a different solution can be considered with reference to the old one. | Search closed as well as open issues. Add changed evidence to the original discussion, or cross-link a genuinely different proposal. Do not make newcomers rediscover the same tradeoff. |
| [VS Code issue grooming](https://github.com/microsoft/vscode/wiki/Issue-Grooming#out-of-scope-feature-requests) | Makes scope and implementation cost explicit when closing requests, with a route to reconsider them. Its page includes historical vote/time thresholds aimed at its much larger queue. | Keep an honest reason and a way back. Do not import those thresholds, bulk closures or automation into a small project with a different community and capacity. |
| [GitHub's maintainer guidance](https://opensource.guide/best-practices/) | Publish project goals and contribution expectations so scope decisions are understandable before somebody invests effort. | Put the explanation at the contributor entry point and beside the relevant proposal, rather than relying on private chat or an agent's memory. |

The [considered-ideas section](../commodity-strategy.md#keep-wider-ideas-with-a-reason-to-revisit-them) applies this to choices already researched here. Its recommendations are still under review in #66; none records an unverified maintainer vote or automatically closes an issue. Existing issues hold work and decisions; their linked source documents hold evidence and implemented contracts.

A useful note answers a future contributor's likely question in a few sentences: what was considered, why the current route is preferable, which evidence supports that view and what would make us change it. Say when a judgment is an untested assumption. Update the existing note at a real decision point and preserve the earlier rationale. Do not manufacture an exhaustive list of ideas, an ADR for every edit, or a recurring review ceremony merely to fill a document.
