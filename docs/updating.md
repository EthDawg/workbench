...
- Run the final extracted app executable with `--check-readback-resources`. It creates and removes only a disposable synthetic session, verifying packaged Snap & Talk skill lookup, exact companion bytes and reopening without microphone/screen access. Packaging also runs this check on the component and after signed Preview identity conversion. `--check-readback-pack` also verifies optional pack installation, snapshots and preservation using private temporary data in the signed Preview and final extracted archive. These checks do not replace the installed New session UI check.
