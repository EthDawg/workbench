...
The normal window makes the app discoverable. The menu bar, keyboard and optional persistent floating toolbar accelerate familiar work. The toolbar uses the same window and saved position for idle actions, dictation and Snap & Talk narration, returning to its idle presentation after the operation. At rest it is a small indicator; hover reveals quick actions with a soft spring and fade, and click expands and remembers the full tools. Minimise stays collapsed until a fresh hover; window resizing does not count as pointer movement. Reduce Motion makes disclosure immediate. Drag empty header space to snap into the nearest named position, or use Position in the toolbar menu. The resting pill turns vertical on the left/right sides and stays horizontal along the top/bottom; every drop docks, including a release near the middle of a display. The native Change menu selects Dictate, Snap & Talk, Annotate, Present or Read controls without ending independent work. The separate text-style menu selects Original, Light or Natural cleanup and the next dictation’s destination. Compact menu-bar actions start each job directly. The native Annotate menu and Tools action expose drawing tools, ink, boards, undo/redo and configured shortcuts, with Finish Drawing preserving the canvas. Window → Focus floating toolbar provides explicit keyboard access without making ordinary pointer interaction activate the app. It hides during screenshot acquisition. Window → Show floating toolbar provides a fallback when macOS conceals a menu-bar item. Closing Home leaves the utility running; Quit must stop capture, playback, drawing and presentation. Login launch is an explicit user setting.
