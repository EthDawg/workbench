...
- Shortcuts are presenter-first. On by default: Dictate ⌃⌥Space, Quick controls ⌃⌥V and Snap & Talk ⌃⌥\\; Draw ⌃⌥S, Highlighter ⌃⌥H and Arrow ⌃⌥A (hold to mark, release to return to the demo), Undo ⌃⌥Z and Clear ⌃⌥X; Persona Overlay ⌃⌥P, with ⌃⌥, and ⌃⌥. to switch cards. Everything else starts off and is one recording away in Keyboard. Defaults avoid the Control-Option window keys that Rectangle and Magnet ship (C, D, E, F, G, I, J, K, R, T, U and the arrows), because only one app can hold a key. Every global shortcut includes Control or Option: a combination such as ⌘S or ⌘T belongs to the app in front, so Workbench never registers one. An update moves only shortcuts still on an old default or on such an app command. A new default never takes a combination someone chose.
