...
- **Menu bar:** a compact quick panel with fixed primary rows: Dictate, Read, Snap & Talk, Draw, Present, Persona Overlay and Timer. Shortcut labels open the existing conflict-checking editor in place. Dictate options put Destination before Text Style and include Recent Transcripts. Open Workbench, Settings and Shortcuts are direct footer actions. A fixed feedback well holds clipboard receipts and errors without moving the action rows. The stack icon stays recognisable; update status comes from the actual updater.
