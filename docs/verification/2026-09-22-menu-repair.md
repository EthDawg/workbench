...
## Microphone finding and resolution

The Mac’s default input was its built-in microphone while its lid was closed. Apple’s hardware disconnect disables that microphone in this state. The initial microphone checks therefore received silence. After the user opened the lid, the configured shortcut started a recording and the installed app produced and saved a transcript. Changed focus correctly caused clipboard fallback. The microphone heard surrounding speech rather than only the synthetic speaker sample, so this establishes a working capture/recognition/save/delivery path, not a controlled live accuracy score. Imported synthetic audio remains the word-for-word accuracy check. No microphone permission, input-device selection or system protection was changed. [Apple’s hardware documentation](https://support.apple.com/guide/security/secbbd20b00b/web).
...
Final local Preview build `20260921221407` contains source `3d6746a86c45ce18c2292b850800b31084af1575`, including the clearer Sound → Input/open-lid error message. The original working draft and its original-text metadata were restored while retaining the verification captures in history. The final bundle was installed after another normal quit.
