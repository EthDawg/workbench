...
Final installed candidate: source `8b3035aedf2a6e5a9c54dd3b4a14ffd9bf055eb5`, Preview 2.0.0 build `20260924141354`, local development, modified source **no**. Native Copy build details confirmed this on macOS 26.5.1 (25F80). All 118 saved files matched an immediate pre-install SHA-256 snapshot before launch. Installed deep/strict signature verification passed, including the browser host and nested Sparkle code. A managed-sandbox verification error was resolved by the same read-only command outside that sandbox; no signing or permission change was needed.

On that final candidate, Start → Pause retained 04:53 and the quick row correctly described **Show or hide timer · 04:53**. Stop returned it to Start Timer. Closing Home retained the floating utility. System appearance, the right-centre anchor, Keep open Off, idle toolbar Off and the originally disabled test shortcuts were restored. Preview was quit and process absence checked before explicitly handing exclusive native ownership to the release coordinator. One named synthetic saved prompt remains for manual insertion acceptance.
...
The installed pass did not exercise live device/persona coexistence, active reading controls or a new Snap & Talk capture session. Their source checks passed; this record does not substitute that result for installed workflow acceptance. The release coordinator owns final production identity, exact merged-source packaging, real Stable-to-Stable update acceptance and publication.
