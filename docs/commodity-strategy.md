...
Read aloud gains seeking in existing audio; annotation boards gain copy/PNG export; device scenes gain a normal resizable presentation option; the separate break timer recovers its saved position; saved resources gain a contextual Return action with truthful copy feedback. Backdrop replacement now preserves a prepared scene and repairs missing images through an explicit preview/apply operation. These use existing playback, renderer, window and library ownership. The timer adds one bounded placement record; no new model, network service or application framework is introduced.
