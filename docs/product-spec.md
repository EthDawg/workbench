...
| Floating toolbar | Shown on first launch; remembered Show floating toolbar setting | Tool glyph or one hover/Keep open row, then recording and result in one window | Shares one saved position. Hidden during screenshot acquisition; returns to tools when done. Hiding idle tools never hides active recording controls |
...
Size is a visible labelled slider in both single-card and multiple-overlay floating controls, with a requested display-width percentage from 6–40%; tall artwork remains height-bounded. Multiple overlays expose Add and Remove beside the selected card's size. Add is scoped to prepared members, disables at eight copies, and remains available after removing the final copy. Remove targets only the selected live instance. The library exposes Add persona, Add / remove members and a confirmed Remove saved persona action; hiding an overlay is distinct from removing saved artwork. No archive/schema change or automatic layout save is introduced.
