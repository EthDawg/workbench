...
New sessions default to the neutral deck skill, which supports an explicitly supplied `template.pptx`. Snap & Talk offers **Install ServiceNow pack**: one action installs the versioned bundle locally and selects ServiceNow under **New session style**. That selection is remembered. Each new branded session includes the complete pack: `SKILL.md`, `brand/` artwork and geometry, `scripts/` local Python helpers, and `requirements.txt`. Employees install through Workbench rather than distributing individual skill files. The existing Hand off action uses the session's complete payload; folder access and prompt submission remain explicit.

`ReadbackSkillPackStore` owns the edition-specific installed copy and validates its file list and hashes. `skill-pack.json` is the immutable session provenance record; `session.json` retains its existing schema. New sessions snapshot the selected pack before publication. Reinstalling or removing an installed pack, changing the future-session style, reopening, or updating Workbench never rewrites old/custom sessions. A missing optional pack cannot break neutral creation. If ServiceNow is explicitly selected but missing or changed, creation stops before making files and offers reinstall or Neutral rather than silently substituting a style. Packaging runs `--check-readback-pack` against the transformed Preview and final extracted production archive using temporary preferences and sessions.

Workbench does not install Python dependencies or execute pack helpers. The helpers preserve eligible manifest order, original screenshot bytes/aspect ratio and exact edited narration in notes; visible titles/takeaways summarise narration. Covers, dividers, summaries and closing slides are opt-in. Non-16:9 captures and short/repeated narration are retained. Unsafe/missing/deleted/unready inputs are reported, not silently substituted. Output creation refuses existing files, and a conservative text-fit check rejects overflow rather than truncating narration. Portable decks deliberately use Arial; explicit `font_mode: "brand"` retains the original ServiceNow font names for environments with those fonts. Fonts are referenced, not bundled. Every finished deck still needs rendering in the receiving environment. Real-session examples and personal glossary corrections are not distributed.
