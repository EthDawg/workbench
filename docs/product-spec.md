...
1. Idle: the optional floating toolbar rests as a small indicator. Hover reveals Dictate, the current dictation cleanup mode, Snap & Talk and Expand. Leaving collapses after a short grace period; an open native menu or drag retains the controls. Click the indicator or Expand to keep all tools open, including Draw and Present. Collapse returns to the indicator; both show/hide and explicit expansion are remembered. Change mode selects Original, Light or Natural cleanup with a native checkmark, separate from the highlighted menu row. Settings remain frozen for an active capture. Window → Focus floating toolbar explicitly enables keyboard access to its native action menu; Escape leaves that interaction. Ordinary hover/click remains nonactivating. Actual configured shortcuts appear beside actions; disabled or failed bindings are labelled honestly.
