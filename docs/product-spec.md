...
### Browser setup default tabs · development candidate

Extension 0.2.1 adds profile-local default-tab controls in Browser setup and an explicit toolbar **Open default tab** action, available without the Mac companion. The manifest changes New Tab to a local Workbench page. Automatic New Tab redirection and an extra profile-startup tab require separate opt-ins, both initially off. Off retains the Workbench page; disable/remove the extension to restore Chrome’s original page (unless another extension controls it) and stop its connection. Existing tabs remain intact. Chrome startup/restored pages can duplicate the optional extra tab. The [browser setup contract](browser-setup.md) owns state, privacy, competing-extension limitations and pending two-profile acceptance. This candidate has not replaced Preview 4 or the store draft.
