...
Recent transcripts can hand off explicitly selected captures using a compatible skill. Each capture retains its original and cleaned wording and is marked as dictated instructions or reference material. The user can explicitly include live referenced screenshot/narration pairs from a chosen Snap & Talk session. `TranscriptHandoff` atomically publishes `SKILL.md`, `handoff.json`, `inputs/` and `outputs/`; unrelated captures and deleted evidence are excluded. It reuses the existing copy-prompt/open-assistant handoff and never submits or executes the request. `--check-transcript-handoff` exercises this with synthetic data during packaging.
