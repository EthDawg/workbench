...
1. Idle: the optional floating toolbar rests as a small indicator. Hover reveals the selected tool, its primary action, actual shortcut and Expand. Change tool selects Dictate, Snap & Talk, Annotate, Present or Read; selection only changes controls. The compact menu-bar panel offers all five primary actions directly, one click each. It fits its content instead of reserving a fixed-height panel; a running action becomes its own finish control. Extra controls are for text style/destination, capture review, drawing tools and scenes. There is no six-dot grip. Snap & Talk shows the active session's non-deleted capture count, Capture next and Review. Dictation's separate text-style menu offers Original, Light or Natural and a destination: Paste automatically or Copy to clipboard. Settings are frozen for an active capture. Click the indicator or Expand to keep contextual controls open. Leaving collapses after a short grace period; a native menu or drag retains the controls. Collapse does not reopen from window resizing. Reveal respects Reduce Motion. Drag the unmarked header space or choose one of eight named positions; saved free placement resolves to its nearest dock. The resting pill is vertical at Left/Right centre; side hover retains the whole pill. Visibility and explicit expansion are remembered. Window → Focus floating toolbar enables keyboard access to its native menu; Escape leaves that interaction. Ordinary hover/click remains nonactivating. Disabled or failed shortcuts are labelled honestly; click a shortcut to open its settings.
