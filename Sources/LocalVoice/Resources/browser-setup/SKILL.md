...
- Extension 0.2.1: copy the role’s default URL into that profile’s Workbench Browser setup. The toolbar can open it explicitly; optional New Tab redirection and an extra startup tab start off. Installing 0.2.1 itself replaces New Tab with a local Workbench page. Turning redirect off keeps that page; disable/remove Workbench to restore Chrome’s original page (unless another extension controls it), also stopping its connection. Startup can duplicate Chrome’s own restored/startup pages. Configure each profile separately; applying a pack never changes these choices. Earlier extensions need Chrome startup settings or a separate New Tab extension. Do not install or configure extensions without task authority.
