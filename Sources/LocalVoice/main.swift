...
final class AppDelegate: NSObject, NSApplicationDelegate, NSPopoverDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        popover.contentViewController = NSHostingController(rootView: WorkbenchQuickPanel(model: model, readback: readback, open: { [weak self] page in self?.navigate(page) }, draw: { [weak self] in
    ...
    func registerShortcuts() {
        ...
        readback?.setShortcutFailure(hotkeys.failures[4])
    }
    func editShortcut(_ id: UInt32) {
        ...
        recorderMonitor = NSEvent.addLocalMonitorForEvents(matching: .keyDown) { [weak self] event in
            ...
            if shortcut.enabled && [UInt32(1), 2, 3, 4].contains(where: { $0 != id && self.model.preferences.shortcut($0) == shortcut }) { self.model.shortcutRecordingMessage = "That shortcut is already assigned in Workbench."; return nil }
