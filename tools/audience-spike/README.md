...
For the first receiver trial, put a synthetic `PRIVATE QA SENTINEL` in any separate, unselected notes window. Share only the audience output to a receiving participant, then bring the notes window forward and overlap the chosen sources. Its absence from the receiver is the relevant check. Workbench is not required for this trial.

Smart Menu notes and profile-following artwork were part of a separate historical presenter experiment; they are not supplied by this PR or current main. Only after a supported implementation exists should a later integration trial substitute that surface for the generic notes window. Do not install the old combined experiment just to run this test. Native persona overlays remain omitted even if visible on the presenter's desktop; neither this trial nor a manual persona fixture establishes their inclusion in the audience output. Preserve unrelated user preferences and libraries.
